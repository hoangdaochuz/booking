...
export function StripePaymentForm({ clientSecret, amount, onSuccess, onError, disabled }: StripePaymentFormProps) {
  ...
  // If publishable key is not provided, show an explanatory message instead of rendering Elements
  if (!publishableKey) {
    return (
      <div className="p-4 bg-yellow-50 border border-yellow-100 rounded">
        <p className="text-sm text-yellow-800">
          Stripe is not configured for the frontend. Card input is disabled because
          <strong> NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY</strong> is not set.
        </p>
        <p className="text-xs text-muted mt-2">To enable payments locally, add the publishable key to <code>.env.local</code> in the <code>frontend/</code> folder and restart the dev server:</p>
        <pre className="text-xs bg-background p-2 rounded mt-2">NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY=pk_test_&lt;your_key&gt;</pre>
      </div>
    );
  }
